...
#include "/afs/cern.ch/user/s/shuangyu/cmsstyle/src/cmsstyle.H"
#include "/afs/cern.ch/user/s/shuangyu/cmsstyle/src/cmsstyle.C"
...
private:
  ...
  void SetupPads()
  {
    topPad = new TPad("topPad", "Top Pad", 0, 0.4, 1, 1);
    bottomPad = new TPad("bottomPad", "Bottom Pad", 0, 0, 1, 0.4);
  ...
  void InitRatioHists(int ratioindex = 0)
  {
    ...
    empty_hist2->GetXaxis()->SetLabelSize(0.1);
    empty_hist2->GetXaxis()->SetTitleSize(0.1);
    empty_hist2->GetXaxis()->SetTitleOffset(0.3);
    empty_hist2->GetYaxis()->SetTitle("#frac{MC}{Data}");
    empty_hist2->GetYaxis()->SetLabelSize(0.1);
    empty_hist2->GetYaxis()->SetTitleSize(0.1);
...
public:
      ...
      : hists(inputs), hists_stack(inputs_stacks)
  {
    ...
    empty_hist1->GetYaxis()->SetLabelSize(0.07);
  ...
  void SetXRange(double min, double max)
  {
    ...
    if (haveratio)
    {
      for (int i = 0; i < hists_ratio.size(); i++)
      {
        hists_ratio.at(i)->GetXaxis()->SetRangeUser(min, max);
      }
    }
  }
  ...
  // for extra line
  void AddExtraLine(double xlow, double ylow, double xup, double yup, bool isratio = false, int color = kBlack)
  {
    cc->cd();
    if (haveratio)
    {
